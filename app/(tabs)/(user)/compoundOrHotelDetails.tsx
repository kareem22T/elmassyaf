...
const HotelDetailsScreen = () => {
  ...
  useEffect(() => {
    const fetchCompound = async () => {
      try {
        const response = await api.get(API_URL + "/api/user/home/" + type + "/" + id);
        setCompound(response.data[type]); // Assuming the data is directly useful
      } catch (err) {
...
function getMidpoint(coordinates) {
  ...
  useEffect(() => {
    if (compound) {
        setImages(compound?.images);
        setSelectedImage(compound?.images[0]);
    }
    }, [wishlist, compound])
    ...
    <SafeAreaView style={styles.container}>
      <StatusBar style="dark" />
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle} medium>تفاصيل {type == 'hotel' ? 'الفندق' : 'المجمع السكني'}</Text>
      ...
      <ScrollView style={styles.scrollView}>
        ...
        <View style={styles.hotelInfo}>
            <View style={{
                ...
                <Text style={styles.hotelName} bold>{compound?.name}</Text>
                <Text style={styles.hotelName} bold>{compound?.base_code}</Text>
            </View>

            {
                compound?.description && (
                    <View style={{
                        ...
                        <WebView
                            originWhitelist={['*']}
                            source={{ html: htmlContent(compound?.description) }}
                            javaScriptEnabled
                            domStorageEnabled
                            injectedJavaScript={`
                ...
                compound?.features && (
                    <View style={{
                        ...
                        <WebView
                            originWhitelist={['*']}
                            source={{ html: htmlContent(compound?.features) }}
                            javaScriptEnabled
                            domStorageEnabled
                            injectedJavaScript={`
                                window.ReactNativeWebView.postMessage(document.documentElement.scrollHeight);
                ...
                (compound && compound.coordinates) && (
                    <View style={{
                        ...
                        <View style={styles.locationRow}>
                            ...
                            <Text style={styles.locationText}>{compound?.address}</Text>
                        </View>
                        <View style={{
                            height: 180,
                            width: '100%',
                            ...
                            mapType='satellite'
                            region={{
                                latitude: parseFloat(compound.coordinates[0]?.lat),
                                longitude: parseFloat(compound.coordinates[0]?.lng),
                                latitudeDelta: 0.1,
                                longitudeDelta: 0.1,
                            }}
                            onPress={() => {}}
        ...
        {
            (compound && compound.units && compound.units.length) ? (
                <View style={{
                    padding: 10,
                    paddingVertical: 20,
                    backgroundColor: '#fff',

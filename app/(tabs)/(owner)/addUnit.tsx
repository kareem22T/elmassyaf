...
const validateForm = () => {
  ...
  if (currentStep >= 4) {
    for (let i = 0; i < cancellationPolicies.length; i++) {
      ...
      if (!policy.days) {
          Toast.show({
            type: 'error',
            text1: 'خطأ في البيانات',
            text2: `ادخل الشرط الاول ${i + 1} من ساسة الالغاء`,
            position: 'top',
            visibilityTime: 3000,
          });
          return false;
      ...
      if (!policy.percentage) {
          Toast.show({
            type: 'error',
            text1: 'خطأ في البيانات',
            text2: `ادخل الشرط الثاني ${i + 1} من ساسة الالغاء`,
            position: 'top',
            visibilityTime: 3000,
          });
          return false;
    ...
    for (let i = 0; i < longTermPolicies.length; i++) {
      const term = longTermPolicies[i];
      if (!term.days) {
          Toast.show({
            type: 'error',
            text1: 'خطأ في البيانات',
            text2: `ادخل عدد ايام حجز اكثر للسياسة ${i + 1}`,
            position: 'top',
            visibilityTime: 3000,
          });
          return false;
      ...
      if (!term.percentage) {
          Toast.show({
            type: 'error',
            text1: 'خطأ في البيانات',
            text2: `ادخل نسبة الخصم للسياسة للسياسة ${i + 1}`,
            position: 'top',
            visibilityTime: 3000,
          });
          return false;
  ...
  if (currentStep >= 5) {
    ...
    for (let i = 0; i < specialPrices.length; i++) {
      const discount = specialPrices[i];
      if (!discount.startDate || !discount.endDate) {
        Toast.show({
          type: 'error',
          text1: 'خطأ في البيانات',
...
const handleSubmit = () => {
  ...
  if (validateForm()) {
    ...
    rooms.forEach((room, roomIndex) => {
      Form.append(`rooms[${roomIndex}][bed_count]`, room.bedsNumber);
      ...
      bed_sizes.forEach((bed, bedIndex) => {
        Form.append(`rooms[${roomIndex}][bed_sizes][${bedIndex}]`, bed);
      });
      ...
      checkedFeatures.forEach((id, index) => {
        Form.append(`rooms[${roomIndex}][amenities][${index}]`, id);
      });
    });
  ...
  return (
    <SafeAreaView style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle} medium>اضافة وحدة</Text>
        ...
        {
          !loading && !error && (
            <SuccessScreen title={'تم رفع اعلان وحدتك بنجاح'} sub_title={'سوف يتم ابلاغك فور نزوله'} visible={modalVisible} onClose={() => router.replace('/(tabs)/(owner)/home')} />
          )
        }
      </SafeAreaView>
  );
